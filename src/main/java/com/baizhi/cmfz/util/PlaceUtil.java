...
//如果返回的是开始时间和结束时间，则递归调用本函数查找随机值 
    ...
    public static User getUser(){
        ...
        cmfzUser.setName(getChineseName());
        cmfzUser.setNickname("kjkjk");
        cmfzUser.setTelphone(getTel());
        cmfzUser.setPassword("123456");
        cmfzUser.setUserStatus(0);
        cmfzUser.setGuruId(1);

        cmfzUser.setSex(sexs[getRandom(sexs.length)]);
        ...
        cmfzUser.setAutograph("sddfdf");

        cmfzUser.setUserImage("jhjhj.jpg");

        cmfzUser.setCreateTime(randomDate("2018-11-20","2019-01-09"));
        return cmfzUser;
    }
